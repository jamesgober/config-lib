...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("  📋 App: {}", conf_config.get("app_name").unwrap().as_string()?);
    println!("  🔧 Debug: {}", conf_config.get("debug").unwrap().as_bool()?);
    ...
    #[cfg(feature = "json")]
    {
        ...
        println!("  📋 App: {}", json_config.get("app_name").unwrap().as_string()?);
        println!("  🏠 Host: {}", json_config.get("server.host").unwrap().as_string()?);
        
        // Demonstrate JSON serialization
        let serialized = json_config.serialize()?;
        println!("  💾 Serialized back to JSON:");
        println!("  {}", serialized.lines().take(3).collect::<Vec<_>>().join("\n  "));
    }
    ...
    #[cfg(feature = "toml")]
    {
        ...
        println!("  📋 App: {}", toml_config.get("app_name").unwrap().as_string()?);
        println!("  🗄️  DB URL: {}", toml_config.get("database.url").unwrap().as_string()?);
    }
    ...
    #[cfg(feature = "noml")]
    {
        ...
        println!("  📋 App: {}", noml_config.get("app_name").unwrap().as_string()?);
        println!("  🌍 Environment: {}", noml_config.get("environment").unwrap().as_string()?);
        println!("  🗄️  DB Host: {}", noml_config.get("database.host").unwrap().as_string()?);
        println!("  🔗 Connection: {}", noml_config.get("connection_string").unwrap().as_string()?);
        
        // Native types are converted to basic types
        println!("  📏 Max file size (bytes): {}", noml_config.get("max_file_size").unwrap().as_integer()?);
        println!("  ⏱️  Timeout (seconds): {}", noml_config.get("timeout").unwrap().as_float()?);
    }
    ...
    println!("  Before merge - Port: {}", base_config.get("port").unwrap().as_integer()?);
    base_config.merge(&override_config)?;
    println!("  After merge - Port: {}", base_config.get("port").unwrap().as_integer()?);
    println!("  Added logging file: {}", base_config.get("logging.file").unwrap().as_string()?);
    println!("  Preserved logging level: {}", base_config.get("logging.level").unwrap().as_string()?);
