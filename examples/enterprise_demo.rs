...
use config_lib::{ConfigManager, EnterpriseConfig, Value};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "✅ Timeout (from default): {}",
        config
            .get_or_default("server.timeout")
            .unwrap()
            .as_integer()
            .unwrap()
    );
    println!(
        "✅ Max Requests (from default): {}",
        config
            .get_or_default("server.max_requests")
            .unwrap()
            .as_integer()
            .unwrap()
    );
...
"#;
    ...
    if let Value::Table(table) = value {
        println!(
            "✅ System: {}",
            table.get("name").unwrap().as_string().unwrap()
        );
        println!(
            "✅ Concurrency: {}",
            table.get("concurrency").unwrap().as_integer().unwrap()
        );
        println!(
            "✅ File Access: {}ns",
            table.get("file_access_time").unwrap().as_integer().unwrap()
        );
        println!(
            "✅ Efficiency: {}%",
            (table.get("efficiency").unwrap().as_float().unwrap() * 100.0)
        );
    }
