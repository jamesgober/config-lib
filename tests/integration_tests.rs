...
fn test_conf_parsing() {
    ...
    // Basic values
    assert_eq!(config.get("app_name").unwrap().as_string().unwrap(), "test-app");
...
fn test_json_parsing() {
    ...
    assert_eq!(config.get("app_name").unwrap().as_string().unwrap(), "json-app");
...
fn test_noml_parsing() {
    ...
    assert_eq!(config.get("app_name").unwrap().as_string().unwrap(), "noml-app");
    assert_eq!(config.get("port").unwrap().as_string().unwrap(), "9000");
    assert_eq!(config.get("host").unwrap().as_string().unwrap(), "example.com");
    
    // Native types converted to basic types
    assert_eq!(config.get("file_size").unwrap().as_integer().unwrap(), 10485760); // 10MB in bytes
    assert_eq!(config.get("timeout").unwrap().as_float().unwrap(), 30.0); // 30 seconds
    
    // Variable interpolation resolved
    assert_eq!(config.get("url").unwrap().as_string().unwrap(), "http://example.com:9000/api");
}
...
fn test_config_modification() {
    ...
    assert_eq!(config.get("section.nested").unwrap().as_bool().unwrap(), true);
...
fn test_value_conversions() {
    ...
    config.set("str_float", "3.14").unwrap();
...
fn test_config_merging() {
    let mut base = Config::from_string(
        "a = 1\nb = 2\n[section]\nx = 10",
        Some("conf")
    ).unwrap();
...
fn test_schema_validation() {
    ...
    // Valid configuration
    let valid_config = Config::from_string(
        "name = test\nport = 8080\ndebug = true",
        Some("conf")
    ).unwrap();
    ...
    let invalid_config = Config::from_string(
        "name = test\ndebug = true",
        Some("conf")
    ).unwrap();
...
fn test_serialization_roundtrip() {
    ...
    assert_eq!(reparsed.get("section.nested").unwrap().as_bool().unwrap(), true);
}
...
fn test_edge_cases() {
    ...
    assert_eq!(config.get("unicode").unwrap().as_string().unwrap(), "Hello World");
}
